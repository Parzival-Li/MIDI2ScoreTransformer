...
class UnpairedXMLDataset(Dataset):
    ...
    def __init__(
        ...
        augmentations: Dict[str, Union[float, Dict[str, float]]] = {},
        return_paths: bool = False,
        id: str = "unpaired_xml_2026_02_18",
    ...
    def _make_surrogate_x(self, y: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        encoder input format must match MultistreamTokenizer.bucket_midi() keys:
        onset(200), duration(200), pitch(128), velocity(8), pad(T)
        """
        T = y["pitch"].shape[0]
        x = {
            "onset": torch.zeros((T, 200), dtype=torch.float32),
            "duration": torch.zeros((T, 200), dtype=torch.float32),
            "pitch": y["pitch"].clone().float(),
            "velocity": torch.zeros((T, 8), dtype=torch.float32),
            "pad": y["pad"].clone().long(),
            "unconditional": torch.ones((T, 1), dtype=torch.float32),
        }
        return x
