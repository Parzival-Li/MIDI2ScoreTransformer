...
class MultistreamTokenizer:
    ...
    def parse_mxl(mxl_path) -> Dict[str, torch.Tensor]:
        ...
        if len(mxl_list) == 0:
            ...
            absolute_onset_stream = torch.Tensor([])
            ...
            # score-side local offset (within measure), used by y
            offset_stream = note_offsets - measure_offsets

            # absolute onset on the unfolded score timeline, used to build surrogate x
            absolute_onset_stream = note_offsets.clone()
        ...
        return {
            ...
            "absolute_onset": absolute_onset_stream,
