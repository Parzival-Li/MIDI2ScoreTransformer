...
class UnpairedXMLDataset(Dataset):
    ...
    def __init__(
        ...
        augmentations: Dict[str, Union[float, Dict[str, float]]] = {},
        return_paths: bool = False,
        id: str = "unpaired_xml_2026_04_16",
        surrogate_qpm: float = 120.0,
        ...
        self.id = id
        self.surrogate_qpm = float(surrogate_qpm)
    ...
    def _random_crop_notes(self, cont: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        Crop by note indices to seq_length. If shorter, keep full (padding later).
        """
    ...
    def _make_surrogate_x_cont(self, cont: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        Build a mechanical surrogate MIDI stream directly from the unfolded XML notes.
        ...
        sec_per_quarter = 60.0 / self.surrogate_qpm

        # XML timing is in quarterLength units; convert to pseudo-MIDI seconds
        onset_sec = cont["absolute_onset"].clone().float() * sec_per_quarter
        duration_sec = cont["duration"].clone().float() * sec_per_quarter

        # After random crop, reset the cropped sequence so the first note starts at 0
    ...
    def _get_one(self, idx: int):
        ...
        # 2) transpose on continuous streams (so that accidental/keysignature stay consistent)
        if self.augmentations.get("transpose", False):
            max_semitones = int(self.augmentations["transpose"])
            shift = random.randint(-max_semitones, max_semitones)

            # we reuse ASAPDataset._transpose(shift) but it expects:
            # midi_stream (pitch) + mxl_stream (pitch) + accidental + keysignature
            # Here we don't have midi, so just pass mxl pitch for both.
            _midi_pitch, mxl_pitch, acc2, ks2 = ASAPDataset._transpose(
                shift,
                midi_stream=cont["pitch"].clone().long(),
                mxl_stream=cont["pitch"].clone().long(),
                accidental_stream=cont["accidental"].clone(),
                keysignature_stream=cont["keysignature"].clone(),
            )
            cont = dict(cont)
            cont["pitch"] = mxl_pitch
            cont["accidental"] = acc2
            cont["keysignature"] = ks2

        # 3) random crop by notes
        cont = self._random_crop_notes(cont)

        # 4) build mechanical surrogate x at continuous level
        x_cont = self._make_surrogate_x_cont(cont)

        # 5) bucket x and y separately
